...
import { recordVerifiedDonation, subscribeToFunding } from '../services/fundingService';
 ...
 * `recordDonation` persists a verified donation (funding total, activity
 * log, donor log, and admin notification, written atomically) and the
 * tier/vessel updates in real time as the subscription pushes the new total
 * back down.
 */
export function useFundingProgress() {
  ...
  return { earned, tier, progressPercent, isMaxTier, nextGoal, lastDonation, recordDonation };
}